...
import { Loader2, Zap, HelpCircle, Sparkles, Info } from "@/lib/icons";
import {
  HEX_SIZE,
  HEX_WIDTH,
  HEX_HEIGHT,
...
const HexTile = React.memo<{
  ...
  ({
    ...
    const IconComponent = isAsking ? HelpCircle : isAutoExpanding ? Zap : Icon;
          ...
          <div className="relative z-10 flex flex-col items-center gap-1 pointer-events-none px-2.5 max-w-[150px]">
            {isLoading ? (
              ...
              <>
                ...
                <span className="text-hex-node font-bold line-clamp-4 text-center text-foreground break-normal [overflow-wrap:break-word] [text-wrap:balance] [hyphens:auto]">
                  {node.text}
                </span>
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.presenceColors.every(
        (c, i) => c === nextProps.presenceColors[i]
      )
    );
  }
);
...
export const HexCanvas = React.memo<HexCanvasProps>(
  ({
    ...
    const getNodeHierarchyLevel = useMemo(
      () =>
        (node: HexNode, isHovered: boolean): number => {
    ...
    const getHierarchyScale = useMemo(
      () =>
        (level: number): number => {
    ...
    const getHierarchyStrokeWidth = useMemo(
      () =>
        (level: number): number => {
            ...
            if (nodes[relatedKey]) {
              ...
              lines.push({
                ...
                color: isCrossCluster
                  ? "rgb(168, 85, 247)"
                  : "rgb(96, 165, 250)", // purple-500 for bridges, blue-400 for related
                isDashed: true,
    ...
    return (
      <>
        ...
        >
          ...
          {connectionLines.map(line => (
            <line
              ...
              strokeLinecap="round"
              strokeDasharray={
                line.isBridge ? "6 8" : line.isDashed ? "8 4" : undefined
              }
        ...
        {Object.entries(nodes).map(([key, node]) => {
          ...
          const isAsking =
            !!node.shouldAskClarifyingQuestion && !answeredAskNodes.has(key);
          ...
          const hierarchyLevel = getNodeHierarchyLevel(
            node,
            hoveredForHierarchy
          );
