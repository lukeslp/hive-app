...
function makeNode(overrides: Partial<HexNode> & { q: number; r: number; text: string }): HexNode {
  return {
    type: 'concept',
    depth: 1,
    pinned: false,
...
describe('Weighted Board Context Scoring', () => {
  ...
  it('scores user-expanded nodes with +3', () => {
    const expanded = makeNode({ q: 0, r: 0, text: 'Expanded', wasInteracted: true });
  ...
  it('scores nodes with contextInfo with +1', () => {
    const withContext = makeNode({ q: 0, r: 0, text: 'Context', contextInfo: 'some notes' });
    expect(scoreNode(withContext)).toBe(1);
  });

  it('scores nodes with relatedNodeKeys with +1', () => {
    const withRelated = makeNode({ q: 0, r: 0, text: 'Related', relatedNodeKeys: ['0,1'] });
  ...
  it('sorts by weight descending, then distance ascending', () => {
    ...
    const sorted = nodes.sort((a, b) => b.weight - a.weight || a.distance - b.distance);
...
describe('Board Context Filtering', () => {
  it('identifies nodes from different clusters as bridge candidates', () => {
    const nodes: Record<string, HexNode> = {
      ...
      '1,0': makeNode({ q: 1, r: 0, text: 'Same Cluster', clusterId: 'A', isKeyTheme: true }),
      '5,0': makeNode({ q: 5, r: 0, text: 'Other Starred', clusterId: 'B', isKeyTheme: true }),
      '6,0': makeNode({ q: 6, r: 0, text: 'Other Expanded', clusterId: 'B', wasInteracted: true }),
      '7,0': makeNode({ q: 7, r: 0, text: 'Other Plain', clusterId: 'B' }),
    };

    const center = nodes['0,0'];
    const otherCluster = Object.entries(nodes)
      .filter(([key, node]) => node.clusterId !== center.clusterId && scoreNode(node) >= 3)
